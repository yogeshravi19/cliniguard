...
The 4 signals are **combined** using a Logistic Regression model that learns the best weights:

```
Risk Score = α × Med-ISP + β × C-AAS + γ × Med-EEM + δ × CDT
```

Weights (α, β, γ, δ) change per **clinical task type**:
...
### The Model
ONE unified CLINIGUARD model trained on **ALL 6 datasets combined** (2,161 rows total).
Split: 70% training / 30% testing. ONE model — tested on all datasets.
